...
cinema.addEventListener('change', (e) => {
    const text = e.target.value;
     if (text == 'Zabrze') {
        ...
        price.textContent = 0;
        for (let i = 1; i <= 3; i++) {
            film[i].textContent =cinemaZabrze[i - 1];
            film[i].value = cinemaZabrze[i - 1];
            hall.textContent = '';
        };
        for (let i = 0; i < hallZabrze.rows; i++) {
            hall.innerHTML += `Rząd ${i+1} `;
            for (let j = 0; j < hallZabrze.columns; j++) {
        ...
        price.textContent = 0;
        for (let i = 1; i <= 3; i++) {
            film[i].textContent = cinemaKraków[i - 1];
            film[i].value = cinemaKraków[i - 1];
            hall.textContent = '';
        };
        for (let i = 0; i < hallKraków.rows; i++) {
            hall.innerHTML += `Rząd ${i+1} `;
            for (let j = 0; j < hallKraków.columns; j++) {
                const div = document.createElement('div');
                div.textContent = `${j+1}`;
                div.dataset.row = i + 1;
                div.dataset.place = j + 1;
    ...
    else if (text == 'Gliwice') {
        liczba = 0;
        price.textContent = 0;
        for (let i = 1; i <= 3; i++) {
            film[i].textContent = cinemaGliwice[i - 1];
            film[i].value = cinemaGliwice[i - 1];
            hall.textContent = '';
        };
        for (let i = 0; i < hallGliwice.rows; i++) {
            hall.innerHTML += `Rząd ${i+1} `;
            for (let j = 0; j < hallGliwice.columns; j++) {
                const div = document.createElement('div');
                div.textContent = `${j+1}`;
                div.dataset.row = i + 1;
                div.dataset.place = j + 1;
